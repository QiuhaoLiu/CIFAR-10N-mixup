...
# 现在训练的是rand1数据集，以cleanlab argmax guessed为参考标准
...
def apply_selective_mixup(inputs, noisy_labels, true_labels, indices, mismatch_set,
                          clean_label_to_indices, clean_train_dataset, use_cuda, alpha):
    """
    对错标样本进行 Mixup 操作。
    ...
    if current_mismatch:
        ...
        # 获取对应的清洁样本
        clean_samples = []
        for label in true_labels_mismatch:
            label = label.item()
            clean_indices = clean_label_to_indices[label]
            if len(clean_indices) == 0:
                raise ValueError(f"No clean samples found for label {label}")
            ...
            if use_cuda:
                clean_image = clean_image.cuda()
            clean_samples.append(clean_image.unsqueeze(0))  # 添加批次维度
        clean_samples = torch.cat(clean_samples, dim=0)  # 形状为 (num_mismatch, C, H, W)

        # 生成混合比例 lambda，确保 lambda >= 0.5
        lam = np.random.beta(alpha, alpha)
        #lam = max(lam, 1 - lam) #保证 lambda >= 0.5 原因是什么？因为保留错标样本的更多特征
        #lam = 1 #我不做混合，是不是训练准确度会进一步提升
        
        # 对错标样本进行 Mixup
        mixed_inputs = lam * inputs[current_mismatch] + (1 - lam) * clean_samples
        inputs[current_mismatch] = mixed_inputs #mixed_x

        # 更新标签为真实标签
        y_a[current_mismatch] = noisy_labels[current_mismatch]
        y_b[current_mismatch] = true_labels_mismatch
        #y_a 是本身标错的标签, y_b 是true_labels
...
def main():
    ...
    if(args.use_docta):
        ...
        cleanlab_argmax_pred_labels = np.load("CIFAR10N\cifar-10-100n\cross_validated_predicted_labels\cifar10_trainset_pyx_argmax_predicted_labels_random_label1_5fold.npy",
                   allow_pickle=True)
        ...
        train_dataset.train_labels = cleanlab_argmax_pred_labels.tolist()  # 假设 train_labels 是列表
        clean_train_dataset.train_labels = cleanlab_argmax_pred_labels.tolist()  # 假设 train_labels 是列表
        print(f"Sample cured_labels: {cleanlab_argmax_pred_labels[:10]}")  # 打印前10个 cured_labels
    ...
    def train(epoch):
        ...
        for batch_idx, (inputs, noisy_labels, true_labels, indices) in enumerate(train_loader): #inputs: img torch.Size(batch_size, channel , W, H); noisy_labels==true_labels: torch.Size(batch_size); ret_index是图片对应的名称索引，比如说img0001
            ...
            inputs, y_a, y_b, lam = apply_selective_mixup(
                ...
                clean_label_to_indices, clean_train_dataset, use_cuda, args.alpha
            )
